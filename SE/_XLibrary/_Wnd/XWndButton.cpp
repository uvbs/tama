...
void XWndButton::Draw( void )
{
	...
	if( m_bLock )
	{
		XBREAK( m_idActLock == 0 );		// 락을 쓰려면 아이디가 정해져있어야 한다.
		pSprObj->SetAction( m_idActLock, xRPT_DEFAULT );
	...
	} else
	{
		...
		{		// disable
			if( m_idActDisable < 0 )		// disable action이 딱히 정해져 있지 않다면.
			{
				if( m_bSprNum==FALSE )
				{
...
BOOL XWndButton::IsWndAreaIn( float lx, float ly )
{
	...
	} else
	{
		...
		if( vScale.x == 0 || vScale.y == 0 )
			vScale = XE::VEC2( 1.0f );
		BYTE a=0;
		m_pSprObj->GetPixel( 0, 0, lx / vScale.x, ly / vScale.y, &a );
		if( a > 0 )
			return TRUE;
	}
	return FALSE;
...
//	m_bToolTip = FALSE;
//	TOOLTIP = NULL;
	if( m_bFirstPush && m_bEnable )	// ButtonDown이 this버튼에서 일어났었을때만 처리 this 버튼영역에 x, y가 있는지 검사
	{
		m_bPush = !m_bPush;
		CallEventHandler( XWM_CLICKED );
		idWnd = GetID();
