...
: XTransition( bHighReso, 1.0f )
{
	m_nType = nTransIn;
	m_nSpeed = nSpeed;
	if( m_nType == 1 )
		m_nXPos = GRAPHICS->GetScreenWidth();
	else 
		m_nXPos = 0;
	SetpSurface1( pFrom );
	SetpSurface2( GRAPHICS->CreateSurface( bHighReso, GRAPHICS->GetScreenWidth(), GRAPHICS->GetScreenHeight(), 0, 0, NULL ) );
...
void XSlideInOut::Draw( void )
{
	...
	} else 
	{
		...
		m_nXPos += m_nSpeed;
		if( m_nXPos >= GRAPHICS->GetScreenWidth() )				// 화면 왼쪽까지 다 이동했으면 완료
			SetComplete( TRUE );		
	}
	XTransition::Draw();
}
...
{
	BYTE alpha = 0;
	if( m_nTransIn )		// fade in- 어두움 -> 밝아짐
		alpha = 255 - (BYTE)(255 * GetfSlerp());
	else					// fade out - 밝음 -> 어두움
		alpha = (BYTE)(255 * GetfSlerp());
	//
	GRAPHICS->FillRect( 0, 0, XSCREEN_WIDTH, XSCREEN_HEIGHT, XCOLOR_RGBA( 0, 0, 0, alpha ) );
	XTransition::Draw();
}
//---------------------------------------------------------------------------------------------------------------
//#ifdef WIN32
...
BOOL XOverlap::Create( void )
{
	if( XE::IsEmpty( m_szImgTo ) )		// 이미지파일이 없으면 이전화면 프레임버퍼에서 새화면 프레임버퍼로 오버랩되는것이다.
		...
		SetpSurface2( GRAPHICS->CreateSurface( GetbHighReso(), m_szImgTo ) );	
	return 1;
}
...
void XOverlap::Draw( void )
{
	XTransition::Draw();		// 가장먼저 불러줘야 함
	{
		...
		float sx = GRAPHICS->GetScreenWidth() / (float)GRAPHICS->GetPhyScreenWidth();
		float sy = GRAPHICS->GetScreenHeight() / (float)GRAPHICS->GetPhyScreenHeight();
