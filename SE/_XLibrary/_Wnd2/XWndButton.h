...
//	XWndTextString( LPCTSTR szString, LPCTSTR szFont, float sizeFont, XCOLOR col=XCOLOR_WHITE );
	...
	XWndTextString( const XE::VEC2& vSize, LPCTSTR szString, XBaseFontDat *pFontDat, XCOLOR col )
		: XWnd( NULL, 0, 0 ) {
			Init();
			XBREAK( szString == NULL );
			SetText( szString );
...
public:
	...
	template<typename T>
	XWndTextStringArea( T x, T y, T w, T h, LPCTSTR szString, XBaseFontDat *pFontDat, XCOLOR col ) 
		: XWnd( NULL, x, y, w, h ) {
		Init();
		XBREAK( szString == NULL );
		SetText( szString );
	...
	XWndTextStringArea( const XE::VEC2& vPos, const XE::VEC2& vSize, LPCTSTR szString, XBaseFontDat *pFontDat, XCOLOR col )
