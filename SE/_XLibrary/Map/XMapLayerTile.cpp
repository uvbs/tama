...
XMapLayerTile::XMapLayerTile( XMap *pMap, int w, int h, XTilePack *pTilePack ) 
: XMapLayer( pMap, xTYPE_TILE, 0, NULL )
{ 
	Init(); 
	// 타일 단위 맵 사이즈
...
void XMapLayerTile::CreateVertexBuffer( XMap *pMap )
{
	XBREAK( m_pTilePack == NULL );
	int tw, th;
	m_pTilePack->GetTileSize( &tw, &th );
//	int num = (int)((GRAPHICS->GetScreenWidth() / tw) + 2) * (int)((GRAPHICS->GetScreenHeight() / th) + 2);		// 뷰포트사이즈에 들어가는 타일개수에 가로세로 2개씩 여분을 더 둠.
...
void XMapLayerTile::FrameMove( float dt )
{
	if( m_pMapDat == NULL )		return;
	//
}

void XMapLayerTile::Draw( void )
{
...
void XMapLayerTile::DrawForMultiPage( void )
{
	// 타일페이지 vram에 올리기
	// 
}
