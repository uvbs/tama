...
float WINAPI DXUTGetFPS()                                  { return GetDXUTState().GetfFPS(); }
...
void	DXUTSetHWnd( HWND hWnd, HINSTANCE hInstance )
{
	GetDXUTState().SetWindowCreateCalled( true );
	GetDXUTState().SetHInstance( hInstance );
	int x = CW_USEDEFAULT;
...
bool DXUTGetDeviceLost( void )
{
	return GetDXUTState().GetDeviceLost();
}

void DXUTOnExitSizeMove( void )
{
