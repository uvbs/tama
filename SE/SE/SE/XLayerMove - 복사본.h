...
*/
struct CHANNEL_POS {
	XE::VEC2 vPos;			// 현재좌표(중심점으로부터의 x,y좌표)
	SPR::xtLINE typePath;	// 패스의 라인종류(라인/스플라인...)
	xSpr::xtInterpolation interpolation;	// 패스의 보간방법
// 	XKeyPos *pStartKey;		// 패스의 시작키
// 	XKeyPos *pEndKey;		// 패스의 끝키
 	XE::VEC2 vStart;		// 시작좌표(랜덤요소때문에 키에서 직접 참조하지 않고 사본을 받음)
 	XE::VEC2 vEnd;			// 끝좌표
	...
	void Clear() {
		vPos.Set(0);
		typePath = SPR::xLINE_NONE;
		interpolation = xSpr::xNONE;
//		pStartKey = pEndKey = nullptr;
		fStartKeyFrame = fEndKeyFrame = 0;
	}
};
...
struct CHANNEL_EFFECT {
	...
	void Clear() {
		...
		DrawMode = xDM_NORMAL;
		interpolation = xSpr::xNONE;
		fAlphaDest = fAlphaSrc = fNextKeyFrame = fStartKeyFrame = 0;
	}
};
...
{
public:
	static BOOL CalcInterpolation( float *pfOut, float fTimeLerp, xSpr::xtInterpolation inter );
...
public:
	...
	// 	}
	float Getx() const { return m_cnPos.vPos.x; }
	float Gety() const { return m_cnPos.vPos.y; }
	const XE::VEC2& GetPos() const {
		return m_cnPos.vPos;
	}
	void Setxy( float x, float y ) {
		m_cnPos.vPos.Set( x, y );
	}
...
// 	}
	...
	virtual void Draw( float x, float y, const D3DXMATRIX &m ) {};
