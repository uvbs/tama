...
// win32환경의 어딘가에서 #define max(a,b)로 되어있는곳이 있어서 그것을 없애는 코드
#ifdef max
#undef max
#endif
#ifdef min
...
BOOL CCoolDialogBar::Create(CWnd* pParentWnd, CDialog *pDialog, CString &pTitle, UINT nID, UINT nToolID, DWORD dwStyle) 
{
...
CSize CCoolDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	if (IsFloating())
	{
		...
		{
		case HTTOPLEFT:
			m_sizeFloat.cx = std::max((int)(window_rect.right - cursor_pt.x), (int)m_cMinWidth) - m_cxBorder;
			m_sizeFloat.cy = std::max((int)(window_rect.bottom - m_cCaptionSize - 
		...
		case HTTOPRIGHT:
			m_sizeFloat.cx = std::max((int)(cursor_pt.x - window_rect.left),
				m_cMinWidth);
			m_sizeFloat.cy = std::max((int)(window_rect.bottom - m_cCaptionSize - 
				cursor_pt.y),m_cMinHeight) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = std::min(cursor_pt.y,
		...
		case HTBOTTOMLEFT:
			m_sizeFloat.cx = std::max((int)(window_rect.right - cursor_pt.x),
				m_cMinWidth) - m_cxBorder;
			m_sizeFloat.cy = std::max((int)(cursor_pt.y - window_rect.top - 
	...
	if (dwMode & LM_LENGTHY)
        return CSize(m_sizeFloat.cx,
            m_sizeFloat.cy = std::max((int)m_sizeMin.cy, nLength));
    else
        return CSize(std::max((int)m_sizeMin.cx, nLength), m_sizeFloat.cy);
}
