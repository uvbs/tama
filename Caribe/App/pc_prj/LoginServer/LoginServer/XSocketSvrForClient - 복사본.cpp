...
XSocketSvrForClient::XSocketSvrForClient( unsigned short port ) 
	: XEWinSocketSvr( _T("로긴서버:클라가 접속하는 서버"), port, MAX_READY_CONNECT, MAX_CONNECT, TRUE ) 
{
	SOCKET_CLIENT = this;
...
// virtual, 커넥션 객체를 생성함. 
XEWinConnectionInServer* XSocketSvrForClient::tCreateConnectionObj( SOCKET socket, LPCTSTR szIP )
{
	// 커넥션 객체 생성
...
void XSocketSvrForClient::OnConnectFromClient( XEWinConnectionInServer *pConnect )
{
...
XEUser* XSocketSvrForClient::CreateUser( XEWinConnectionInServer *pConnect )
{
