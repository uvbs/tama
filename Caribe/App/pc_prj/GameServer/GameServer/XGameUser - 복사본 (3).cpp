...
//		int apAdd = secOffline / secRegenAP;
		...
		int apAdd = (int)(secOffline * XGC->m_apPerOnce);
...
// 			pSpot->SetPower( power );
			...
			_tstring strLog = XE::Format(_T("Spot Recon Result Enemy(XGAME::xSPOT_JEWEL) User_Level: %d, idxMine: %d, Spot_Level: %d, Mine_Level: %d, Name: %s")
																			, GetLevel(), idxMine, infoMatch.m_lvAcc, infoMatch.m_LvMine, infoMatch.m_strName);
			AddLog(XGAME::xULog_User_Spot_Recon, strLog);
			// 클라에 패킷은 이미 앞에서 다 보냄
			// 클라에 보냄
			{
...
//	bool bSend = true;
	...
	if( battle.bCheatKill ) {
		...
		const auto errCodeKill = m_spAcc->IsAbleKill( battle.idSpot, pBaseSpot->GetPower(), pBaseSpot->IsNpc() );
...
/**
 @brief 유저의 길드관련 정보 업데이트 요청
*/
// void XGameUser::RecvGuildUserUpdate( XArchive& p )
// {
// 	XArchive ar;
// 	XAccount::sSerializeGuildInfo( m_spAcc, &ar );
// 	SendGuildUserEvent( xGEV_GUILD_INFO, ar );
// }
/**
 @brief 길드이벤트를 클라로 알린다.
*/

// void XGameUser::SendGuildUserUpdate( xtGuildEvent event, xtBitGuildUpdate bitUpdate )
// {
// 	XArchive arParam;
// 	XGuild::sUpdateByEvent( event, )
// }
...
// #ifdef _XUZHU
							CONSOLE_ACC( "jewel", "보석광산: 첫 침공당함/방어성공" );
//#endif // _XUZHU
							const bool bSimul = true;
							DoMatchJewel( pSpot, bSimul, bWin );
						} else {
//#ifdef _XUZHU
							CONSOLE_ACC( "jewel", "보석광산: 재 침공당함/방어성공" );
//#endif // _XUZHU
							// 내꺼였으며 매치상대가 있었던 경우.
						...
						if( pSpot->GetidMatchEnemy() == 0 ) {
//#ifdef _XUZHU
							CONSOLE_ACC( "jewel", "보석광산: 첫 침공당함/방어실패" );
//#endif // _XUZHU
							// 내꺼였으며 매치상대가 없었던 경우.
							const bool bSimul = true;
							DoMatchJewel( pSpot, bSimul, bWin );
...
//#ifdef _XUZHU
							CONSOLE_ACC( "jewel", "보석광산: 재 침공당함/방어실패" );
//#endif // _XUZHU
							// 내꺼였으며 매치상대가 있었던 경우.
							// 상대에게 뺏긴걸로 처리한다.
							ID idDBAcc = pSpot->GetidMatchEnemy();
