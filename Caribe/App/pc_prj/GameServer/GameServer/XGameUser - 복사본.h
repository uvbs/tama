...
public:
	...
	void SuccessLoginAfterSend( void );	
	//
	GET_SET_ACCESSOR( XSPSAcc, spAcc );
//	GET_SET_ACCESSOR( BOOL, bDestroySave );
//	GET_SET_ACCESSOR( XLegion*, pEnemyLegion );
...
// 	int RecvCashItemBuyApple(XPacket& p);			//애플 인앱 결제
	int RecvCashItemBuyGoogle(XPacket& p);			//구글 인앱 결제
	...
	void cbSuccessLoginAfterSendResult(XPacket& p);
...
//	void SendBattleInfo( ID idPacket, ID idSpot, ID idAcc, XLegion *pLegion, XArchive *pParam=NULL );
	void SendBattleInfo( bool bRecon/*, ID snSession*/, ID idAccEnemy, XSpot *pSpot, XArchive *pParam = nullptr);
...
// 	int RecvLevelUpSkill( XPacket& p );
	...
	int DoSpotAttackCampaign( XSpot *pBaseSpot, int idxStage );
	...
	void DoMatchSulfur( XSpotSulfur *pSpot );
...
//	void RecvShopCashCallResult(XPacket& p);
	...
	void RecvAccountPostDeleteResult(XPacket& p);
...
//	int SendResearchComplete( ID idPacket, xErrorCode codeError );
	...
	void SendSubscribePackage();
	void AddPostSubscribe();
...
private:
	...
	void SimulAttackedHome( ID idAcc, int lvAcc, const _tstring& strName, int power, int ladder );
	void SimulAttackedJewel( XSpotJewel *pSpot, bool bDefenseOk, const XGAME::xJewelMatchEnemy& info );
#endif // _DEV
//	void ArchivingSendIngameFinishBattle( XPacket& ar, const XGAME::xBattleResult& result );
};
