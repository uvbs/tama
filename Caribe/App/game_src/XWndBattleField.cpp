...
{
	return ( m_spSelectSquad != nullptr ) ? m_spSelectSquad->GetsnSquadObj() : 0;
}
...
int XWndBattleField::Process( float dt )
{
	...
	if( m_spSelectSquad != nullptr && m_spSelectSquad->IsLive() == FALSE )
		m_spSelectSquad.reset();
	// 임시선택중이던 부대가 죽으면 선택취소
	if( m_spTempSelect != nullptr && m_spTempSelect->IsLive() == FALSE )
	{
		m_spTempSelect.reset();
		m_vTouch.Set(-1.f);
	}
	// 
	if( m_spTempSelectEnemy != nullptr && m_spTempSelectEnemy->IsLive() == FALSE )
...
// 	}
	...
	if( m_spTempSelect != nullptr || 
		m_spTempSelectEnemy != nullptr || 
		m_spSelectSquad != nullptr )
		m_vTouch.Set( lx, ly );
...
/**
 ...
 @param camp 픽킹조건진영
*/
SquadPtr XWndBattleField::PickingSquad( float lx, float ly, BIT bitCamp )
{
	// 윈도우 좌표를 월드좌표로 변환
