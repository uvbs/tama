...
void XScenePatchClient::Create( void )
{
	...
	m_LocalLastUpdate = GetUpdateVer( _T(FILE_LAST_UPDATE) );
...
*/
int XScenePatchClient::GetUpdateVer( LPCTSTR szRes )
{
	CToken token;
	// 워크에서 읽는다.
	if( token.LoadFromWork( szRes, XE::TXT_EUCKR ) == xFAIL )
	{
		XLOGXN("GetUpdateVer:LoadFromWorld failed:%s", szRes );
		return 0;
	}
	token.GetToken();		// Ver
	int ver = token.GetNumber();
  XLOGXN( "GetUpdateVer:%d", ver );
...
int XScenePatchClient::FSMReqVersion( xtAct event )
{
	...
	case xACT_RECV: {
		...
		// 다운받은 Lastupdate.txt_ 파일을 읽는다.
		{
			...
			int verServer = GetUpdateVer( strVer + _T("_") );
		...
		if( IsFirstPlay() ) {
			// WiFi가 아니냐
			if( XSYSTEM::CheckWiFi() == FALSE ) {
				// 다운로드 받을래?
//				auto pAlert = XWND_ALERT_YESNO( "alert.cdma.check", "%s", XTEXT( 9 ) );
				...
				if( pAlert ) {
					pAlert->SetEvent( XWM_YES, this, &XScenePatchClient::OnYesCDMADownload );
					pAlert->SetEvent( XWM_NO, GAME, &XGame::OnExitApp );
				}
			} else
				FSMReqVersion( xACT_ON_YES );
			m_bPatched = true;
		} else
		// 최초실행이 아님
		{
	...
	case xACT_RECV: {
		...
		for( auto& res : listFulls )
		{
			...
			if( !m_listCompleteFiles.Findp( str ) )
				m_pTask->AddRequest( _T( XFULL_DIR ), res.strFile );
		}
		m_pTask->SetEvent( XWM_ALL_COMPLETE, this,
...
int XScenePatchClient::FSMReqUpdateList( xtAct event )
{
	...
	case xACT_ENTER: {
		...
		for( int i = m_LocalLastUpdate + 1; i <= m_LastUpdateOnServer; ++i ) {
...
// #ifdef _DEV
// 			strBuff = XFORMAT( " core update %d/%d: %s (%d bytes received)", 
