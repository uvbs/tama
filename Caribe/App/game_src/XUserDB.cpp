...
BOOL CUserDB::AccountUpdateBattleInfo(ID idDefender, ID idAttacker, int sectime)
{
	...
	ret = Excute(_T("{ call dbo.AccountUpdateBattleInfo( ?,?,? ) }"));
	if (ret == SQL_SUCCESS) {
		return TRUE;
	} else
	if( ret == SQL_NO_DATA ) {
...
// 	ret = BindCol(SQL_C_WCHAR, szUUID, LEN_LOGIN_ID * 2, &nickLen, i++);
	...
	ret = BindCol( SQL_C_LONG, &numSubScribe, sizeof( int ), &nLen, i++ );
	ret = BindCol( SQL_C_LONG, &_secNextSubscribe, sizeof( int ), &nLen, i++ );
	ret = BindCol( SQL_C_SHORT, &sGMLevel, sizeof( short ), &nLen, i++ );
...
//				}
		...
		// 월정액상품
		if( Ver < 6 ) {
			// 구버전 데이타일경우 기존에 시간값으로 쓰고 있었기땜에 매우 큰값이 들어있음. 0으로 모두 초기화
			numSubScribe = 0;				// 기존데이타
			_secNextSubscribe = 0;
		}
		// offline 횟수를 계산한다.
		spAcc->m_numSubscribe = numSubScribe;		// 월정액 지급중이면 0 > 값이 들어간다. 최초 한번 지급하면서 1이되므로.
		xSec secNextSubScribe = (xSec)_secNextSubscribe;
		const auto secCurr = XTimer2::sGetTime();
		const int secInterval = XGC->m_secSubscribeCycle;
		//
		XBREAK( numSubScribe > 0 && secNextSubScribe == 0 );		// 월정액 지급중인데 다음 지급시간이 없으면 에러임
		// 지급일이 지났으면 offline카운트를 올리고 24시간을 더해서 다시 비교하기를 반복.
		while( secCurr >= secNextSubScribe && secNextSubScribe ) {
			if( ++spAcc->m_cntSubscribeOffline >= 100 )
				break;		// 뭔가 이상한 상황
			secNextSubScribe += secInterval;
		}
		XBREAK( spAcc->m_cntSubscribeOffline > 30 );	// 이런경우도 로그를 남겨봄
		XBREAKF( spAcc->GetcntSubscribeOffline() >= 100, "secNext=%d, numSubscribe=%d", secNextSubScribe, numSubScribe );
		spAcc->m_secNextSubscribe = secNextSubScribe;
		//	
		return xDB_OK;
	} else
	if( ret == SQL_NO_DATA ) {
...
//	spAcc->MakeArchiveHeros(HeroInfoPacket);
	...
	DWORD secNextSubscribe = spAcc->GetsecNextSubscribe();
	int _numSub = spAcc->GetnumSubscribe();
	if( _numSub < 0 )
		_numSub = 0;
...
// 	BindParameter(SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, sizeof(int), 0, &nsecShop, 0, &retLen, sqlcount++);
	...
	BindParameter( SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, sizeof( int ), 0, &numSubscribe, 0, &retLen, sqlcount++ );
	BindParameter( SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, sizeof( int ), 0, &secNextSubscribe, 0, &retLen, sqlcount++ );
...
// 	BindParameter(SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, sizeof(int), 0, &nsecSubscribeEnd, 0, &retLen, sqlcount++);
	...
	BindParameter( SQL_PARAM_INPUT, SQL_C_SHORT, SQL_SMALLINT, sizeof( short ), 0, &sGMLevel, 0, &retLen, sqlcount++ );
...
// 	if( bSimul ) {
// 		_stprintf_s(szQuery, 
// 			_T("{ call dbo.FindAccMineEnemyZeroByDummy( '%d',  '%d',  '%d', '%d' ) }")
...
// 	}
	...
	ret = BindCol(SQL_C_LONG, &info.m_idxLegion, sizeof( int ), &nLen, i++);
	ret = BindCol(SQL_C_SBIGINT, &secPlaying, sizeof(XINT64), &nLen, i++);
	ret = Fetch();
	if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
		XBREAK( pOut == nullptr );
...
//POST
/**
 @brief idAcc의 모든 편지들을 꺼내 arOut에 담는다.
*/
int CUserDB::LoadPostInfoByIdAcc(ID idAcc, XArchive& arOut)
{
	//발신일, 만료일 계산 해서 읽어 들일때 삭제 해야 할 메일은 삭제.
	ReAllocStmt();
	TCHAR szQuery[1024];
	SQLRETURN	ret;
	_stprintf_s(szQuery, _T("{ call dbo.LoadPostInfoByIdAcc( '%d' ) }"), idAcc);
	if ((ret = Excute(szQuery)) != SQL_SUCCESS) {
		TCHAR err[1024];
		ShowErrorInfo(SQL_HANDLE_STMT, err, 1024);
		OutputTextFormat(_T("\n %s Execute Error (err:%d) : %s"), __TFUNC__, ret, err);
...
//	TCHAR		szIdentyfy[LEN_POST_TITLE] = { 0, };
	...
	SQLLEN		sqllenBuffItems = 0;
	BYTE		buffItems[BINARY_BUFF_LEN2];
	::memset(&buffItems, 0x0, BINARY_BUFF_LEN2);

	ret = BindCol(SQL_C_LONG, &snPost, sizeof(ID), &nLen, i++);
	ret = BindCol(SQL_C_WCHAR, szSenderName, LEN_LOGIN_ID * 2, &sendernameLen, i++);
	ret = BindCol(SQL_C_LONG, &Ver, sizeof(int), &nLen, i++);
	ret = BindCol(SQL_C_SHORT, &_posttype, sizeof(short), &nLen, i++);
	ret = BindCol(SQL_C_WCHAR, szTitle, LEN_POST_TITLE, &titleLen, i++);
	ret = BindCol(SQL_C_WCHAR, szMessage, LEN_POST_MSG, &msglen, i++);
	ret = BindCol(SQL_C_TINYINT, &numItems, sizeof(BYTE), &nLen, i++);
	ret = BindCol(SQL_C_BINARY, &buffItems, BINARY_BUFF_LEN2, &sqllenBuffItems, i++);
		
	XVector<XPostInfo> aryPosts;
	// 더이상 없을때까지 편지를 퍼낸다.
	while ((ret = Fetch()) == SQL_SUCCESS || (ret = Fetch()) == SQL_SUCCESS_WITH_INFO)//!= SQL_NO_DATA)
	{
		XPostInfo postInfo( snPost );
		postInfo.SetstrSenderName( szSenderName );
		postInfo.SetstrTitle( szTitle );
		postInfo.SetstrMessage( szMessage );
		if( numItems > 0 ) {
			XArchive arItems;
			arItems.SetBufferMem( buffItems, sqllenBuffItems );
			postInfo.DeSerializeItems( arItems, numItems, Ver );
		}
		aryPosts.Add( postInfo );
	}
	arOut << aryPosts;
	return (int)aryPosts.size();
}
//
/*
