...
{
	XPropItem::xPROP *pProp = PROP_ITEM->GetpProp( szIdentifier );
...
XBaseItem* XBaseItem::sCreateItem( XPropItem::xPROP *pProp, ID snItem )
{
...
XBaseItem::XBaseItem( ID idItem, ID snNum )
{
	Init();	
	m_idProp = idItem;
	_m_pProp = PROP_ITEM->GetpProp( idItem );
...
XBaseItem::XBaseItem( LPCTSTR szIdentifier, ID snNum )
{
	Init();
	//
	_m_pProp = PROP_ITEM->GetpProp( szIdentifier );
	XBREAK( _m_pProp == nullptr );
	if( _m_pProp == nullptr )
		return;
...
XBaseItem::XBaseItem( XPropItem::xPROP *pProp, ID snItem )
{
	Init();
...
*/
bool XBaseItem::IsBetterThan( XBaseItem* pItemEquip )
{
	// 등급이 높으면 일단 좋음.
...
*/
float XBaseItem::GetValAdjParam( XGAME::xtParameter adjParam )
{
// 	XARRAYLINEARN_LOOP_AUTO( GetpProp()->aryAdjParam, &adj )
	for( const auto& adj : GetpProp()->aryAdjParam ) {
