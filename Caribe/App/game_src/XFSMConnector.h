...
enum xtFSMEvent {
	xFE_NONE,
	xFE_START_CONNECTING,			// 현재는 사용하지 않으나 사용해야 할듯.
...
{
public:
	XFSMBaseConnector( xConnector::xtFSMId idFSM ) { 
		Init(); 
		m_idFSM = idFSM;
	}
...
public:
	XFSMOffline() 
		: XFSMBaseConnector( xConnector::xFID_OFFLINE ) {
		Init();
	}
	~XFSMOffline() {
		Destroy();
...
{
public:
	XFSMConnectingG() : XFSMBaseConnector( xConnector::xFID_CONNECTING_GAMESVR ) { Init(); }
...
{
public:
	XFSMLoginingG() : XFSMBaseConnector( xConnector::xFID_LOGINING_GAMESVR ) { Init(); }
...
{
public:
	XFSMOnline() : XFSMBaseConnector( xConnector::xFID_ONLINE ){ Init(); }
...
{
public:
	XFSMDelayAfterConnectingL() 
		: XFSMBaseConnector( xConnector::xFID_DELAY_AFTER_CONNECTING_LOGINSVR ) { Init(); }
...
{
public:
	XFSMDelayAfterConnectingG()
		: XFSMBaseConnector( xConnector::xFID_DELAY_AFTER_CONNECTING_GAMESVR ) { 
		Init(); 
	}
	~XFSMDelayAfterConnectingG() { Destroy(); }
	// get/setter
	// public member
	void Process() override;
protected:
