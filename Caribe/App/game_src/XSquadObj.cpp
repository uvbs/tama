...
class L {
	std::list<shared_ptr<S>> m_listSquad;
	shared_ptr<U> m_spTarget;
	Release() {
		m_spTarget.reset();
		for( auto s : m_listSquad )
			s->Release();
		// m_listSquad는 이곳이 원본(?)이므로 reset을 하지 않는다. L 파괴자에서 자동으로 파괴된다.
		//m_listSquad.clear();		
	}
};

class S {
	std::list<shared_ptr<U>> m_listUnit;
	shared_ptr<S> m_spTarget;
	Release() {
		m_listUnit.clear();		// 이것은 여기가 원본이 아니므로(objmng가 원본)참조 해제
		m_spTarget.reset();0
	}
};

class ObjMng {
	std::list<U> m_listUnit;
	shared_ptr<U> m_spTarget;
	Release() {
		m_spTarget.reset();
// 		for( auto spUnit : m_listUnit )
...
class U {
	shared_ptr<U> m_spTarget;
	shared_ptr<S> m_spSquad;
	shared_ptr<L> m_spLegion;
	Release() {
		m_spTarget.reset();
		m_spSquad.reset();
		m_spLegion.reset();
...
{
	shared_ptr<L> spLegion;
	spLegion->Relase();

	pObjMng->Release();
	SAFE_DELETE( pObjMng );	=> 이때 모든 Unit들이 실제 파괴되길 원함.
}

어떤 클래스(A)가 shared_ptr<B>을 가지고 있을때 B가 원본(?)이고
클래스 종료시에 B를 최종 파괴하고 싶다면
B를 참조하는 다른 모든 객체에 미리  Release를 호출해서 해제를 시킨다.
그리고 A가 파괴될때 refCnt가 1이되면서 실제 파괴되도록 한다.
다시 말해 자신이 파괴될때 최종 파괴하고 싶다면 Release()에서 reset()을 하면 안된다는 얘기
일관되지 않으므로 헷갈릴수 있음
파괴되는곳을 일원화 하고 shared_ptr도 쓰려면 weak_ptr을 쓰는게 가장 낫다.
