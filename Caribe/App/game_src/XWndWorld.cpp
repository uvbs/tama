	...
	: XWndPopup( _T( "option.xml" ), "popup_option" )
{
	...
	if( XAPP->m_bDebugMode ) {
		...
		pButt->SetEvent( XWM_CLICKED, SCENE_WORLD, &XSceneWorld::OnClickDebugShowLog, 2 );
		pButt->SetbCheck( XE::GetbAllowLog() );
		Add( pButt );
		vPos.x += vSize.w + 10.f;
		pButt = new XWndButtonDebug( vPos, vSize, _T( "log" ) );
		pButt->SetEvent( XWM_CLICKED, SCENE_WORLD, &XSceneWorld::OnClickDebugShowLog, 0 );
		Add( pButt );
		vPos.x += vSize.w + 10.f;
		pButt = new XWndButtonDebug( vPos, vSize, _T( "del log" ) );
		pButt->SetEvent( XWM_CLICKED, SCENE_WORLD, &XSceneWorld::OnClickDebugShowLog, 1 );
		Add( pButt );
