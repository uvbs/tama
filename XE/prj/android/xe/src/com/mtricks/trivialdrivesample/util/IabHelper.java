...
package com.mtricks.trivialdrivesample.util;
...
public class IabHelper {
	...
	public Inventory queryInventory(boolean querySkuDetails,
		...
		try {
			...
			logDebug("queryPurchases:1 ");
			int r = queryPurchases(inv, ITEM_TYPE_INAPP);
			if (r != BILLING_RESPONSE_RESULT_OK) {
				logDebug("err queryPurchases:1 ");
			...
			if (mSubscriptionsSupported) {
				logDebug("queryPurchases:2 ");
				r = queryPurchases(inv, ITEM_TYPE_SUBS);
				if (r != BILLING_RESPONSE_RESULT_OK) {
					logDebug("err queryPurchases:2 ");
					throw new IabException(r,
	...
	public void queryInventoryAsync(final boolean querySkuDetails,
		...
		logDebug("1:queryInventoryAsync" );
		checkNotDisposed();
		logDebug("2:queryInventoryAsync" );
		checkSetupDone("queryInventory");
		logDebug("3:queryInventoryAsync" );
		flagStartAsync("refresh inventory");
		logDebug("4:queryInventoryAsync" );
		(new Thread(new Runnable() {
			public void run() {
				logDebug("11:queryInventoryAsync:Thread" );
				IabResult result = new IabResult(BILLING_RESPONSE_RESULT_OK,
						"Inventory refresh successful.");
				logDebug("22:queryInventoryAsync:Thread" );
				Inventory inv = null;
				logDebug("33:queryInventoryAsync:Thread" );
				try {
					inv = queryInventory(querySkuDetails, moreSkus);
				} catch (IabException ex) {
					Log.e("IabHelper", "iap:queryInventoryAsync:Exception" );
					result = ex.getResult();
				}
				logDebug("44:queryInventoryAsync:Thread" );
				flagEndAsync();
				logDebug("55:queryInventoryAsync:Thread" );

				final IabResult result_f = result;
				final Inventory inv_f = inv;
				logDebug("66:queryInventoryAsync:Thread" );
				if (!mDisposed && listener != null) {
					handler.post(new Runnable() {
						public void run() {
							logDebug("77:queryInventoryAsync:Thread" );
	...
	public void consumeAsync(Purchase purchase,
		...
		logDebug("11:consumeAsync");
		checkNotDisposed();
		logDebug("22:consumeAsync");
		checkSetupDone("consume");
		logDebug("33:consumeAsync");
		List<Purchase> purchases = new ArrayList<Purchase>();
		purchases.add(purchase);
		logDebug("44:consumeAsync");
	...
	int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus)
		...
		if (!skuDetails.containsKey(RESPONSE_GET_SKU_DETAILS_LIST)) {
			...
			if (response != BILLING_RESPONSE_RESULT_OK) {
				logError("getSkuDetails() failed: " + getResponseDesc(response));
				return response;
			} else {
	...
	void consumeAsyncInternal(final List<Purchase> purchases,
		...
		logDebug("1:consumeAsyncInternal");
		final Handler handler = new Handler();
		logDebug("2:consumeAsyncInternal");
		flagStartAsync("consume");
		logDebug("3:consumeAsyncInternal");
		(new Thread(new Runnable() {
			public void run() {
				logDebug("11:consumeAsyncInternal:thread");
				final List<IabResult> results = new ArrayList<IabResult>();
				logDebug("11:consumeAsyncInternal:thread");
				for (Purchase purchase : purchases) {
					try {
						logDebug("22:consumeAsyncInternal:thread");
						consume(purchase);
						logDebug("33:consumeAsyncInternal:thread");
						results.add(new IabResult(BILLING_RESPONSE_RESULT_OK,
								"Successful consume of sku "
										+ purchase.getSku()));
					} catch (IabException ex) {
						Log.e("iap", "exception:consumeAsyncInternal:thread");
						results.add(ex.getResult());
					}
				}

				logDebug("44:consumeAsyncInternal:thread");
				flagEndAsync();
				logDebug("55:consumeAsyncInternal:thread");
				if (!mDisposed && singleListener != null) {
					logDebug("666:consumeAsyncInternal:thread");
					handler.post(new Runnable() {
						public void run() {
				...
				logDebug("77:consumeAsyncInternal:thread");
				if (!mDisposed && multiListener != null) {
					logDebug("88:consumeAsyncInternal:thread");
					handler.post(new Runnable() {
						public void run() {
							logDebug("1111:consumeAsyncInternal:thread");
