...
XESocketClientInServer::XESocketClientInServer( const char *cIP, unsigned short port, XNetworkDelegate *pDelegate, int sizeQueue, DWORD param )
...
BOOL XESocketClientInServer::CreateSocket( void )
{
	//
...
void XESocketClientInServer::WorkThread( void ) 
{
//	int sizePacket = 0;
//	int totalRead = 0;
	while(1)
...
void XESocketClientInServer::ConnectThread( void )
{
	SOCKADDR_IN addr; 
	addr.sin_family = AF_INET; 
...
// 접속 시도
BOOL XESocketClientInServer::TryConnect( void )
{
...
// 소켓에 연결이 성공했다
void XESocketClientInServer::CreateWorkThread( void )
{
	Lock(__TFUNC__);
//	DWORD idThread;
	// recv 스레드 생성
...
BOOL XESocketClientInServer::Process( void )
{
