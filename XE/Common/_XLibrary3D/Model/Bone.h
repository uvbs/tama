...
#define		MAX_VS_BONE		9		// VS에서 최대사용가능한 뼈대레지스터의 개수.
...
struct BONE
{
	...
	MATRIX		m_mTM;		// ase원본 TM
	MATRIX		m_mInverseTM;	// * 미리 변환된 인버스TM 
	MATRIX		m_mLocalTM;	// * 미리변환된 로컬 TM
};
...
public:
	...
	MATRIX	m_mLocalRH;		// 오른손 무기의 RHand로부터의 상대TM
	MATRIX	m_mLocalLH;		// 오른손 무기의 LHand로부터의 상대TM
	MATRIX	m_mLocalShield;	// 방패의 LForeArm으로부터의 상대TM
	MATRIX	m_mLocalKnuckle;	// 너클.
...
public:
	...
	char	m_szName[128];				// 모션 파일명 - 나중에 이건 빠진다.
	float	m_fPerSlerp;				//
	int		m_nMaxBone;					// 이것은 어디까지나 확인용 변수일뿐이다.  실제 본의 갯수가 필요하다면 CBone에서 읽어야 한다.
	Vec3	m_vPivot;				// 0,0,0(발)기준에서 골반(루트)의 좌표.  0번키의 좌표와 같다.
	...
	void	ReadTM( FILE *fp, int nNumBone, int nNumFrame );

	void	AnimateBone( MATRIX *pmUpdateBone, CMotion *pMotionOld, float fFrameCurrent, int nNextFrame, int nFrameOld, BOOL bMotionTrans, float fBlendWeight );
