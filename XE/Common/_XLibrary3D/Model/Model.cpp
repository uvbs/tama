...
void CModel::FrameMove( float fSpeed )
{
	if( m_nPause )		
		return;
...
BOOL CModel::Render( const MATRIX* pmWorld )
{
	return TRUE;
}
...
{
	Vec3 vLen = m_vMax - m_vMin;
	float	fLen = MatrixVec3Length( vLen );
	return fLen / 2;
}

BOOL CModel::IntersectBB( const Vec3 &vRayOrig, const Vec3 &vRayDir, const MATRIX &mWorld, Vec3* pvIntersect, FLOAT* pfDist )
{
	...
	for( int i = 0; i < 12 * 3; i += 3 )
	{
		...
		MatrixVec3Multiply( v2, v2, mWorld ); 
		MatrixVec3Multiply( v3, v3, mWorld ); 
