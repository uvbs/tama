...
#include "Lua/5.1/include/lua.h"
#include "Lua/5.1/include/lualib.h"
#include "Lua/5.1/include/lauxlib.h"
#else
...
inline int xlua_error(lua_State *L) {
    return 0;
}
...
//		lua_tinker::def(L, "_ALERT", show_error);
	...
	// 
	void ClearReturn( void ) { m_nNumArg = 0; }
	template<typename T>
	...
	xtResume Resume( const char *szFunc ) {	// szFunc함수를 resume으로 실행한다. NULL이면그냥 resume이다
		if( szFunc && XE::IsHave(szFunc) ) {
	...
	template<typename T1>
	xtResume Resume( const char *szFunc, T1 arg1 ) {	// szFunc함수를 resume으로 실행한다. NULL이면그냥 resume이다
		if( szFunc && XE::IsHave(szFunc) ) {
			lua_pushstring( L, szFunc );
			lua_gettable( L, LUA_GLOBALSINDEX );
			lua_tinker::push( L, arg1 );
	...
	xtResume Resume( const char *szFunc, T1 arg1, T2 arg2 ) {	// szFunc함수를 resume으로 실행한다. NULL이면그냥 resume이다
		if( szFunc && XE::IsHave(szFunc) ) {
			lua_pushstring( L, szFunc );
			lua_gettable( L, LUA_GLOBALSINDEX );
			lua_tinker::push( L, arg1 );
...
#define CONSOLE_FUNCNAME	CONSOLE( "%s", Convert_char_To_TCHAR(__FUNCTION__) );
