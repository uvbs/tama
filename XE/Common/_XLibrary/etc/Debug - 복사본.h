...
#define	XLOG(F,...) __xLog( XLOGTYPE_LOG, _T("%s(%d) %s():\n    ")_T(F), __FILENAME,__LINE__,_T(__FUNCTION__), ##__VA_ARGS__ )
#ifdef WIN32
  #define	XLOGX(F, ...)		XLIB->Console(_T(F),##__VA_ARGS__)
  #define	XLOGXN(F, ...)	XLIB->Console(_T(F)_T("\r\n"),##__VA_ARGS__)
#endif // win32
//Android
#ifdef _VER_ANDROID
  #define	XLOGX(F,...) __xLogx( _T(F), ##__VA_ARGS__ )
  #define	XLOGXN(F,...) __xLogx( _T(F)_T("\r\n"), ##__VA_ARGS__ )
  #define	AXLOGX		XLOGX		// 안드로이드 전용 로그.
  #define	AXLOGXN	XLOGXN
#else
  #define	AXLOGX(F,...)		(0)
  #define	AXLOGXN(F,...)	(0)
#endif
...
#else
    #define XTRACE( F, ... )		XLOGXN( F, ##__VA_ARGS__ )
#endif
...
#ifdef WIN32
  #define CONSOLE( F, ... )		XLIB->Console(_T(F)_T("\n"),##__VA_ARGS__)
#endif
#ifdef _VER_ANDROID
  #define CONSOLE( F, ... )		XLOG(_T(F),##__VA_ARGS__)
#endif
#ifdef _VER_IOS
  #define CONSOLE( F, ... )		_NSLog(_T(F),##__VA_ARGS__)
#endif
...
// 조건 EXP를 만족하면 에러창을 띄움
		...
		#define AfxDebugBreak	DebugBreak
			...
			#define XBREAKF(EXP, F, ...)		(( (EXP) ) ? XLOG( F, ##__VA_ARGS__ ), (AfxDebugBreak(), 1)  : 0)		// 프로그래머 디버거 모드에선 브레이크 잡히니까 경고창띄우는게 필요 없을거 같아서 경고창 뺌
...
#else // XDEBUG
	// 최종마스터 버전
	#define XBREAK(EXP)		(( (EXP) ) ? XLOG( #EXP ), 1  : 0)		// 디버그모드에선 브레이크잡으면서 트레이스 메시지도 띄움
	#define XBREAKF(EXP, F, ...)		(( (EXP) ) ? XLOG( F, ##__VA_ARGS__ ), 1  : 0)		// 프로그래머 디버거 모드에선 브레이크 잡히니까 경고창띄우는게 필요 없을거 같아서 경고창 뺌
	#define XASSERT(EXP)		(( !(EXP) ) ? XLOG( #EXP ), 0  : 1 )
	#define XASSERTF(EXP, F, ...)		(( !(EXP) ) ? XLOG( F, ##__VA_ARGS__ ), 0  : 1)
