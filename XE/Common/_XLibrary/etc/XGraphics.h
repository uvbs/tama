...
public:
	...
	enum xtLine {
		xLINE_NONE = 0, xLINE_POINT, xLINE_LINE
	};
...
public:
	...
	*/
	...
	inline xPixelFormat GetPixelFormat() {
		return m_pixelFormat;
	}
	inline void	SetPixelFormat( xPixelFormat pixelFormat ) {
		m_pixelFormat = pixelFormat;
	}
	inline int		GetBackBufferWidth() {
		return m_nBackBufferWidth;
	}
	inline int		GetBackBufferHeight() {
		return m_nBackBufferHeight;
	}
	inline void	SetBackBufferSize( int w, int h ) {
		m_nBackBufferWidth = w; m_nBackBufferHeight = h;
	}
	inline int		GetWorkBuffereWidth() {
		return m_nWorkBufferWidth;
	}
	inline int		GetWorkBuffereHeight() {
		return m_nWorkBufferHeight;
	}
	inline void	SetWorkBufferSize( int w, int h ) {
		m_nWorkBufferWidth = w; m_nWorkBufferHeight = h;
	}
	inline int		GetPhyScreenWidth() {
		return m_nPhyScreenWidth;
	}
	inline int		GetPhyScreenHeight() {
		return m_nPhyScreenHeight;
	}
	inline XE::VEC2 GetPhyScreenSize() {
		return XE::VEC2( m_nPhyScreenWidth, m_nPhyScreenHeight );
	}
	inline void	SetPhyScreenSize( int w, int h ) {
		m_nPhyScreenWidth = w; m_nPhyScreenHeight = h;
	}
	inline float GetLineWidth() {
		return m_wLine;
	}
	inline float SetLineWidth( float w ) {
		float oldw = m_wLine;
		m_wLine = w;
		return oldw;
	}
	inline float GetRatioWidth() {
		return GetPhyScreenWidth() / GetLogicalScreenSize().w;
	}
	inline float GetRatioHeight() {
		return GetPhyScreenHeight() / GetLogicalScreenSize().h;
	}
	inline XE::VEC2 GetRatioResolution() {
		return GetPhyScreenSize() / GetLogicalScreenSize();
	}
	virtual void*	GetScreenPtr() {
		XLOG( "구현되지 않았습니다" ); return NULL;
	}
	virtual void*	LockBackBufferPtr( int *pWidth, BOOL bReadOnly ) {
		XLOG( "구현되지 않았습니다" ); return NULL;
	}
	virtual void*	GetWorkBufferPtr() {
		XLOG( "구현되지 않았습니다" ); return NULL;
	}
	virtual void*	ReadBackBuffer( int width, int height, void *pBuffer ) {
		XLOG( "구현되지 않았습니다" ); return NULL;
	}
	virtual void		UnlockBackBufferPtr() {
		XLOG( "구현되지 않았습니다" );
	}
	...
	virtual void	ReleaseDrawTarget() {
		XLOG( "구현되지 않았습니다" );
	}
	virtual void RestoreDevice() = 0;
	//
	inline int GetViewportLeft() {
		return m_nViewportLeft;
	}
	inline int GetViewportRight() {
		return m_nViewportRight;
	}
	inline int GetViewportTop() {
		return m_nViewportTop;
	}
	inline int GetViewportBottom() {
		return m_nViewportBottom;
	}

	inline int GetViewportWidth() {
		return m_nViewportRight - m_nViewportLeft + 1;
	}
	inline int GetViewportHeight() {
		return m_nViewportBottom - m_nViewportTop + 1;
	}
	inline XE::VEC2 GetViewportSize() {
		return XE::VEC2( GetViewportWidth(), GetViewportHeight() );
	}
	inline XE::VEC2 GetViewportLT() {
		return XE::VEC2( m_nViewportLeft, m_nViewportTop );
	}
	inline XE::VEC2 GetViewportRB() {
		return XE::VEC2( m_nViewportRight, m_nViewportBottom );
	}
	inline void SetViewportWidth( int vw ) {
		m_nViewportRight = m_nViewportLeft + vw - 1;
	}
	inline void SetViewportHeight( int vh ) {
		m_nViewportBottom = m_nViewportTop + vh - 1;
	}

	inline void SetViewportSize( int w, int h ) {
		SetViewportWidth( w );
		SetViewportHeight( h );
	}
	...
	inline XE::VEC2 ViewportPosToScreenPos( const XE::VEC2& posViewport ) {
...
#pragma mark draw
	...
	void DrawPoint( const XE::VEC2& vPos, float size, XCOLOR col ) {
		DrawPoint( vPos.x, vPos.y, size, col );
	}
	void DrawRect( const XE::VEC2& vLT, const XE::VEC2& vRB, XCOLOR dwColor ) {
		DrawRect( vLT.x, vLT.y, vRB.x - vLT.x + 1, vRB.y - vLT.y + 1, dwColor );
	}
	void DrawRectSize( const XE::VEC2& vLT, const XE::VEC2& vSize, XCOLOR dwColor ) {
		DrawRect( vLT.x, vLT.y, vSize.x, vSize.y, dwColor );
	}
	...
	template <typename T>
	void FillRect( T x, T y, T w, T h, XCOLOR dwColor ) {
		FillRect( (float)x, (float)y, (float)w, (float)h, dwColor );
	}
	...
	void FillRectSize( const XE::VEC2& vLT, const XE::VEC2& vSize, XCOLOR dwColor ) {
		FillRect( vLT.x, vLT.y, vSize.x, vSize.y, dwColor );
	}
	template <typename T>
	void DrawLine( T x1, T y1, T x2, T y2, XCOLOR color ) {
	...
	void DrawLine( const XE::VEC2& v1, const XE::VEC2& v2, XCOLOR dwColor ) {
		DrawLine( v1.x, v1.y, v2.x, v2.y, dwColor );
	}
	void DrawLine( const XE::VEC2& v1, const XE::VEC2& v2, XCOLOR col1, XCOLOR col2 ) {
		DrawLine( v1.x, v1.y, v2.x, v2.y, col1, col2 );
	}
	template<typename T>
	void DrawHLine( T x, T y, T w, XCOLOR dwColor ) {
		DrawLine( x, y, x + w, y, dwColor );
	}
	template<typename T>
	void DrawVLine( T x, T y, T h, XCOLOR dwColor ) {
		DrawLine( x, y, x, y + h, dwColor );
	}
	...
	virtual XSurface* CreateSurface( BOOL bHighReso
																	 ...
																	 , BOOL bSrcKeep = FALSE ) {
		WARNING_NOT_IMPLEMENT; return NULL;
	}
	...
	virtual BOOL LoadImg( LPCTSTR szFilename, int *pWidth, int *pHeight, DWORD **ppImage ) {
		WARNING_NOT_IMPLEMENT; return FALSE;
	}
...
public:
	...
	inline XSurface* CreateSurface( LPCTSTR szRes
		...
		return CreateSurface( true, szRes, formatSurface, bUseAtlas, bSrcKeep, bMakeMask, true );
	}
	...
	// 
	virtual void DrawFan( float *pAryPos, float *pAryCol, int numVertex, int numFan ) {
		WARNING_NOT_IMPLEMENT;
	}
...
inline XGraphics*	GetGraphics() {
	return XGraphics::s_pGraphics;
}
