...
//        XLOG( "경고:%s의 가로크기가 2로 나누어 떨어지지 않습니다", w );
			XALERT( "warning:The horizontal size of the %s divided by 2 does not fall", m_szFilename );
		m_vSize.x = w * __fResoScale; 
		m_vSize.y = h * __fResoScale; 
	}		
public:
	XBaseImage( BOOL bHighReso ) {		// 고해상도리소스로 지정되면 화면에 찍힐땐 실제크기의 반으로 찍힌다
		Init(); 
		m_bHighReso = bHighReso; 
		if( bHighReso )		__fResoScale = 0.5f;
	}
	virtual ~XBaseImage() { Destroy(); }
	// get/set
	GET_ACCESSOR( const XE::VEC2&, vSize );		// 이후 부터는 그냥 꺼낸다
  GET_SET_ACCESSOR(int, bpp);
	int GetWidth( void ) { 
		return (int)m_vSize.x; }
	int GetHeight( void ) { 
		return (int)m_vSize.y; }
	inline int GetMemWidth( void ) { 
		return (int)(m_vSize.x / __fResoScale); }	// 실제메모리크기를 알아내기
	inline int GetMemHeight( void ) { 
		return (int)(m_vSize.y / __fResoScale); }
	XE::POINT GetSizeMem() {
	...
	virtual BOOL Load( LPCTSTR szFilename ) {
        if( szFilename )
            _tcscpy_s( m_szFilename, XE::GetFileName(szFilename) );
        return TRUE;
    }
};
