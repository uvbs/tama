...
- (BOOL)verifyPurchase:(SKPaymentTransaction *)transaction
{
	...
	//jsonObjectString을 서버로 보내서 서버에서 POST로 보내서 검증하는 루틴을 만들것.
...
-(NSString*)getTransactionReceipt:(SKPaymentTransaction *)transaction
{
    BOOL isOk = [self isTransactionAndItsReceiptValid:transaction];
    if (!isOk)
    {
...
-(NSString*)getPurchaseInfo:(SKPaymentTransaction *)transaction
{
	...
	NSData *jsonData = [NSJSONSerialization dataWithJSONObject:purchaseInfoDict options:NSJSONWritingPrettyPrinted error:nil];
	NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
	const char *cJson = [jsonString UTF8String];
	int lenJson = strlen(cJson);
    NSString *encodedString = [self encodeBase64:(const uint8_t*)cJson
                                             length:lenJson];
...
#warning Validation suceeded. Unlock content here.
		NSLog(@"Success Receipt Validation ");
    } else
	{
		NSLog(@"Fail Receipt Validation");
	}
}
