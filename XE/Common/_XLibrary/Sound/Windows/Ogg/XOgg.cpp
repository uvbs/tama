...
void XOgg::Init( void )
{
	...
	InitVorbisFile();
	if (!m_bVorbisInit) {
 		XBREAKF(1, "Failed to find OggVorbis DLLs (vorbisfile.dll, ogg.dll, or vorbis.dll)" );
		ALFWShutdown();
		return;
	}
}
...
//				ulBufferSize = ulFrequency * 3;
		...
		if (ulFormat != 0)
		{
			...
			if (!pDecodeBuffer) {
				XBREAKF(1, "Failed to allocate memory for decoded OggVorbis data\n");
				fn_ov_clear(&sOggVorbisFile);
//				ShutdownVorbisFile();
				return FALSE;
			}
