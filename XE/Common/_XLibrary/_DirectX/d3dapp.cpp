...
#ifdef __USE_NPROTECT
#if __NPROTECT_VER == 3
#include "npgamelib3.h"
	extern	CNPGameLib	npgl;
	extern	CTimer	s_tmnpgl;
#endif	// __NPROTECT_VER
#endif	// __USE_NPROTECT
...
{
#if !defined( __DAISY ) && !defined( __BEAST ) && !defined( __MAKEMAP )
#ifndef _DEBUG
//	InitEH();
#endif
#endif
...
#define XXLOG	/##/
#else
#define	XXLOG(F,...) __xLog( XLOGTYPE_LOG, _T(F), ##__VA_ARGS__ )	
#endif
//-----------------------------------------------------------------------------
// Name: Create()
// Desc:
...
//    ::GetWindowRect( m_hWnd, &m_rcWindowBounds );
  //  ::GetClientRect( m_hWnd, &m_rcWindowClient );
	memset( &m_rcWindowBounds, 0, sizeof(m_rcWindowBounds) );
	memset( &m_rcWindowClient, 0, sizeof(m_rcWindowClient) );
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
 ...
 #if 0
    // Create the device
    hr = m_pD3D->CreateDevice( m_pD3D->GetAdapterCount()-1, D3DDEVTYPE_SW,
...
//								m_hWndFocus,  D3DCREATE_MANAGED, &m_d3dpp,
//								&m_pd3dDevice );
#endif
#else
	hr = m_pD3D->CreateDevice( m_d3dSettings.AdapterOrdinal(), pDeviceInfo->DevType,
    ...
    if( SUCCEEDED(hr) )
    {
		m_pd3dDevice->GetSwapChain( 0, &m_pd3dChain );
...
#ifdef __BEAST // BMP 출력을 위해서 백버퍼를 얻기 위해서 이 기능을 셋.
...
HRESULT CD3DApplication::Reset3DEnvironment()
{
    ...
    if( FAILED( hr = m_pd3dDevice->Reset( &m_d3dpp ) ) )
	{
		...
		{
			XBREAKF( 1, "CD3DApplication::Reset3DEnvironment()에 Reset리턴값 : D3DERR_DEVICELOST가 나왔다." );
		}
...
//#ifdef __CLIENT //!defined(__BEAST) && !defined(__DAISY) && !defined(__SFXEDITOR) && !defined(__SFXEDITOR)
#if defined(__CLIENT) || defined(__COLA) //&& !defined(__SFXEDITOR) && !defined(__SFXEDITOR)
	if( IsDrawTiming() )
#endif
	{
		...
		if( D3DERR_DEVICELOST == hr )
		{
			XBREAKF( 1, "CD3DApplication::Render3DEnvironment()에 Present리턴값 : D3DERR_DEVICELOST" );
			m_bDeviceLost = true;
		}
	}
#ifndef __BEAST
...
void CD3DApplication::Cleanup3DEnvironment()
{
    if( m_pd3dDevice != NULL )
    {
		...
		{
			TCHAR szString[ 256 ];
			_stprintf_s( szString, _T("Device에서 Release되지 않은 갯수 : %d"), nCount );
			MessageBox( NULL, szString, _T("MASQUERADE"), MB_ICONERROR|MB_OK );
