...
class XActObj {
	...
	const XActDat* m_pAction = nullptr;			// 이 노드가 가리키는 SprDat에서의 액션 포인터. read only
	void Init() {
		m_nNumLayers = 0;
		m_ppLayers = NULL;
		m_pSprObj = NULL;
...
// 		m_pSprObj = pSprObj;
// 	}
	XActObj( XSprObj *pSprObj, const XActDat *pAction );
	~XActObj() {
		Destroy();
	}
	// get/set
	GET_ACCESSOR_CONST( const XActDat*, pAction );
	...
	XBaseLayer *CreateLayer( int idx, const LAYER_INFO* pLayerInfo );
	void ClearLayer();
private:
};