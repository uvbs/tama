...
{ 
	Init(); 
	Load( szFilename, XE::VEC3(), FALSE, false ); 
	m_pDelegate = pDelegate; 
}
XSprObj::XSprObj( LPCTSTR szFilename, const XE::VEC3& vHSL, XDelegateSprObj *pDelegate/* = nullptr*/ ) 
{
	Init();
	Load( szFilename, vHSL, FALSE, false );
	m_pDelegate = pDelegate;
}
// for lua
...
{
	Init();
	Load( Convert_char_To_TCHAR(cFilename), XE::VEC3(), bKeepSrc, false );
}
#ifdef WIN32
XSprObj::XSprObj( BOOL bKeepSrc, LPCTSTR szFilename ) 
{
	Init();
	Load( szFilename, XE::VEC3(), bKeepSrc, false );
}
#endif // WIN32
...
void XSprObj::FrameMove( float dt )
{
	...
	if( m_pSprDat == nullptr && lazy.strFilename.empty() == false ) {
		...
		if( lazy.timerLazyLoad.IsOver() ) {
			if( Load( lazy.strFilename.c_str(), lazy.vHSL, lazy.bKeepSrc, false ) ) {
...
#endif
	XBREAK( SPRMNG == NULL );
	m_pSprDat = SPRMNG->Load( szFilename, vHSL, TRUE, bKeepSrc, bAsyncLoad );
	if( m_pSprDat == nullptr ) {
