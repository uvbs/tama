...
//		auto sizeParent = pParentValid->GetSizeFinal();
		const auto sizeParent = pParentValid->GetSizeLocalNoTrans();
...
*/
void XWndTextString::PreCalcLayoutSize() 
{
	if( XE::IsEmpty( m_szString ) ) {
		m_sizeString.Set( 0 );
		return;
...
XE::VEC2 XWndTextString::GetSizeForceFinal()
{
	XE::VEC2 vSize;
...
#ifdef _VER_OPENGL
					vAdj.y = ( size.h * 0.5f ) - ( (m_sizeString.h * vScale.y) * 0.5f );	// 윈도우의 중앙에서 다시 스트링크기의 절반을 빼준다.
#endif // _VER_OPENGL
				} 
				if( m_Align & XE::xALIGN_HCENTER ) {
					if( m_sizeString.IsValid() == false )
...
#ifdef _VER_OPENGL
					vAdj.x = ( size.w * 0.5f ) - ( (m_sizeString.w * vScale.x) * 0.5f );	// 윈도우의 중앙에서 다시 스트링크기의 절반을 빼준다.
#endif // _VER_OPENGL
				}
...
#endif // win32
				const XE::VEC2 vPos = v + GetvAdjDrawFinal();
...
int XWndTextString::Process( float dt )
{
	...
	auto pCompAlpha = m_compMngByAlpha.FindComponent( "alpha" );
	if( pCompAlpha ) {
