...
{
	char temp[64];
	sprintf_s(temp, "%llu", *(long long*)lua_topointer(L, 1));
...
{ 
	...
	{ 
		...
		case LUA_TTHREAD : 
		case LUA_TBOOLEAN : 
				CCONSOLE("(%d:%s)%d ", i, lua_typename(L, lua_type(L, i)), lua_toboolean(L, i)); break; 
		case LUA_TLIGHTUSERDATA : 
//				CCONSOLE("(%d:%s) ", i, lua_typename(L, lua_type(L, i))); break; 
				CCONSOLE("(%d:%s) 0x%08x", i, lua_typename(L, lua_type(L, i)), (unsigned long)lua_touserdata(L,i) ); break; 
		case LUA_TUSERDATA : 
			CCONSOLE("(%d:%s) 0x%08x", i, lua_typename(L, lua_type(L, i)), (unsigned long)lua_touserdata(L,i) ); break; 
		case LUA_TNUMBER : 
				CCONSOLE("(%d:%s)%d ", i, lua_typename(L, lua_type(L, i)), (int)lua_tonumber(L, i)); break; 
		case LUA_TSTRING : 
				CCONSOLE("(%d:%s)%s ", i, lua_typename(L, lua_type(L, i)), lua_tostring(L, i)); break; 
		case LUA_TFUNCTION : 
			{
				lua_CFunction func = lua_tocfunction(L,i);
				CCONSOLE("(%d:%s) 0x%08x", i, lua_typename(L, lua_type(L, i)), (unsigned long)func ); break; 
			}
		} 
	} 
	CCONSOLE("\n"); 
} 
#endif // lua tinker debug
/*---------------------------------------------------------------------------*/ 
/* Tinker Class Helper                                                       */ 
/*---------------------------------------------------------------------------*/ 
...
static void invoke_parent2(lua_State *L)
{
		PrintStack(L);
...
{
	lua_getmetatable(L,1);	// 스택의 bottom에서 메타테이블을 읽어서 스택에 푸쉬한다(-2)
	lua_pushvalue(L,2);		// 스택의 bootom+1에서 값을 가져와 스택에 푸쉬한다(-1)
	lua_rawget(L,-2);			// -2스택의 테이블의 값중에 스택의맨위(-1)값을 키로 뽑고 그값을 스택에 푸쉬한다(-1,value)
		PrintStack(L);
...
bool lua_tinker::member_isfunction( lua_State* L, const char* tn, const char* fn )
{
	PrintStack(L);
	lua_pushcclosure(L, on_error, 0);
	int errfunc = lua_gettop(L);
