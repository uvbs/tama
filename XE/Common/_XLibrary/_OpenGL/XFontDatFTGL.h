...
public:
	...
	static void ToScreenCood( XE::VEC2 *pvInOut ) { 
		...
		{
			pvInOut->x *= (GRAPHICS->GetScreenWidth() / 768.f);	// ex: 480/1024
			pvInOut->y *= (GRAPHICS->GetScreenHeight() / 1024.f);
		}
	}
	...
	static void To1024Cood( XE::VEC2 *pvInOut ) { 
		...
		{
			pvInOut->x *= (768.f / GRAPHICS->GetScreenWidth());
			pvInOut->y = 1024.f - (pvInOut->y * (1024.f / GRAPHICS->GetScreenHeight()));		// 위아래를 뒤집어준다
		}
	}
	// virtual
	virtual float GetFontHeight( void ) { 
		...
		if( GRAPHICS->GetScreenWidth() > GRAPHICS->GetScreenHeight() )
			...
			return GetFontSize() * (GRAPHICS->GetScreenHeight() / 1024.f); 
#else
		return GetFontSize(); 
#endif
	}
	virtual float GetFontWidth( void ) {
		...
		if( GRAPHICS->GetScreenWidth() > GRAPHICS->GetScreenHeight() )
			...
			return GetFontSize() * (GRAPHICS->GetScreenWidth() / 768.f); 
#else
		return GetFontSize(); 
#endif
	}
